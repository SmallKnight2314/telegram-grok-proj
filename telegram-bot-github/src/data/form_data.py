...
class FormData:
    def __init__(self):
        ...
        logger.debug("FormData initialized")
        print("FormData initialized")
    ...
    def store(self, user_id: int, key: str, value: Any) -> None:
        if user_id not in self._data:
            ...
            print(f"Created new data dictionary for user {user_id}")
        self._data[user_id][key] = value  # Store key-value pair for the user
        logger.debug(f"Stored data for user {user_id}: {key} = {value}")
        print(f"Stored data for user {user_id}: {key} = {value}")

    def get_form_data(self, user_id: int) -> Dict[str, Any]:
        logger.debug(f"Retrieving form data for user {user_id}")
        print(f"Retrieving form data for user {user_id}")
        data = self._data.get(user_id, {})  # Return user data dictionary or empty dict if not found
