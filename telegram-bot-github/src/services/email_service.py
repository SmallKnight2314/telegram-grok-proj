    ...
    def send_email(self, form_data: dict, user_id: int, from_email: str, to_email: str, user_name: str, media_path: str = None, retries: int = 2, delay: int = 5) -> bool:
        ...
        try:
            ...
            for attempt in range(retries + 1):
                try:
                    logger.debug(f"Attempt {attempt + 1}: Connecting to SMTP server: {self.smtp_server}:{self.smtp_port}")
                    ...
                    if attempt == retries:
                        logger.warning("Retrying with SSL")
                        break
                    time.sleep(delay)

            # Fallback to SSL (port 465)
            try:
                logger.debug(f"Connecting to SMTP server (SSL): {self.smtp_server}:{self.smtp_port}")
                ...
                logger.error(f"SSL connection failed for user {user_id}: {str(e)}", exc_info=True)
                return False
